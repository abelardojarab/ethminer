...
bool OCLMiner::s_adjustWorkSize = false;
...
vector<int> OCLMiner::s_devices(MAX_MINERS, -1);
...
bool OCLMiner::configureGPU(unsigned _localWorkSize, int _globalWorkSizeMultiplier,
    ...
    try
    {
        ...
        if( s_clKernelName == OCLKernelName::Fpga ) {
            ...
            if (t.good()) {
                ...
                code = ckernel;
                cllog << "OpenCL kernel: Using 'kernel.aocx'";
            } else {
                code = "";
            }
        } else {
        ...
        cl::Program::Sources sources{{code.data(), code.size()}};
        cl::Program program(m_context, sources);
        program = createProgramFromBinary(m_context(), binary_file.c_str(), &(device()), 1);
